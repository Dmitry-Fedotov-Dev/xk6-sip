...
Blind transfer: sends REFER that asks the other party to call `dest`. Call it on the leg with the party to transfer. The method returns when the REFER is accepted; wait for the result with [`expectTransferred()`](../expecttransferred/).
...
- A proxy passes the REFER to the device of the transferred party. The device then places the new call itself; get it with [`expectReferredCall()`](../expectreferredcall/) on that party's leg.

| Parameter | Type | Default | Description |
| --- | --- | --- | --- |
| dest | [Device](../../device/) or string | | Transfer target: a device, a number or a SIP URI. |
