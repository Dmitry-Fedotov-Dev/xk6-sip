...
`Device` is one SIP subscriber, like a desk phone or a softphone. It owns one UDP socket that it uses both to send requests and to receive them. It keeps its registration refreshed and makes and receives [calls](../call/_index.md).

Create devices in the init context. The constructor does no network I/O: the socket is opened and REGISTER is sent on first use inside a test function. A device lives until [`sip.shutdown()`](../shutdown.md) or the end of the test, not only for one iteration, so a VU reuses its devices across iterations.
...
| --- | --- | --- |
| [call( options )](call.md) | no | Starts an outgoing call and returns its [Call](../call/_index.md), or `false`. |
| [expectCall( [options] )](expectcall.md) | yes | Waits for an incoming call that matches the caller and number, and returns it, or `false`. |
| [register()](register.md) | yes | Sends REGISTER now. Returns `true` on success. |
| [isRegistered()](isregistered.md) | no | Whether the last REGISTER succeeded. |
| [identity( key )](identity.md) | no | Returns one of the device numbers, for example `identity('ext')`. |
| [destroy()](destroy.md) | yes | Hangs up the device calls, unregisters it and closes its socket. |
...
- It accepts re-INVITE from the other side, for example hold and resume, and tracks it in [`isRemoteHold()`](../call/isremotehold.md).
...
- When it receives REFER, it places the new call itself. The script gets that call with [`expectReferredCall()`](../call/expectreferredcall.md).
